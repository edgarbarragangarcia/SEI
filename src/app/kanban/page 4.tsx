import KanbanBoard from "@/components/kanban-board";"use client";"use client";
...
const states = ["ATENDIDA", "AGENDADA", "PENDIENTE", "RECHAZA", "NO ASISTIO", "ASISTIO"];const states = ["ATENDIDA", "AGENDADA", "PENDIENTE", "RECHAZA", "NO ASISTIO", "ASISTIO"];
...
const Card = ({ patient }: { patient: any }) => {
  ...
  const [{ isDragging }, drag] = useDrag(() => ({
    ...
    type: ItemTypes.CARD,const Card = ({ patient }: { patient: any }) => {import { Header } from "@/components/header";

    item: { patient },

    collect: (monitor) => ({  const [{ isDragging }, drag] = useDrag(() => ({
  ...
  return (
    ...
    <div    collect: (monitor) => ({const ItemTypes = {  CARD: 'card',

      ref={drag as unknown as React.Ref<HTMLDivElement>}

      className={`p-4 mb-4 bg-white rounded-lg shadow-md ${isDragging ? 'opacity-50' : 'opacity-100'}`}      isDragging: !!monitor.isDragging(),

    >

      <h4 className="font-bold">{`${patient.nombre} ${patient.apellidop} ${patient.apellidom}`}</h4>    }),  CARD: 'card',};
...
const Column = ({ state, patients, onDrop }: { state: string, patients: any[], onDrop: (p: any, state: string) => void }) => {
  ...
  const [{ isOver }, drop] = useDrop(() => ({    <divconst states = ["ATENDIDA", "AGENDADA", "PENDIENTE", "RECHAZA", "NO ASISTIO", "ASISTIO"];

    accept: ItemTypes.CARD,

    drop: (item: { patient: any }) => onDrop(item.patient, state),      ref={drag as unknown as React.Ref<HTMLDivElement>}

    collect: (monitor) => ({

      isOver: !!monitor.isOver(),      className={`p-4 mb-4 bg-white rounded-lg shadow-md ${isDragging ? 'opacity-50' : 'opacity-100'}`}const states = ["ATENDIDA", "AGENDADA", "PENDIENTE", "RECHAZA", "NO ASISTIO", "ASISTIO"];
  ...
  return (      <h4 className="font-bold">{`${patient.nombre} ${patient.apellidop} ${patient.apellidom}`}</h4>const Card = ({ patient, onDrop }: { patient: any, onDrop: (p: any, state: string) => void }) => {

    <div ref={drop as unknown as React.Ref<HTMLDivElement>} className={`w-1/5 p-4 bg-gray-100 rounded-lg ${isOver ? 'bg-gray-200' : ''}`}>

      <h3 className="font-bold mb-4 text-center">{state}</h3>      <p className="text-sm text-gray-600">{patient.email}</p>

      {patients.map((patient, index) => (

        <Card key={index} patient={patient} />      <p className="text-sm text-gray-800">{patient.nhcdefinitivo}</p>const Card = ({ patient }: { patient: any }) => {  const [{ isDragging }, drag] = useDrag(() => ({
...
export default function KanbanPage() {};
  ...
  const [isLoading, setIsLoading] = useState(true);    type: ItemTypes.CARD,    item: { patient },

  const { data: session, status } = useSession();

  const { toast } = useToast();const Column = ({ state, patients, onDrop }: { state: string, patients: any[], onDrop: (p: any, state: string) => void }) => {



  useEffect(() => {  const [{ isOver }, drop] = useDrop(() => ({    item: { patient },    collect: (monitor) => ({

    const fetchData = () => {

      setIsLoading(true);    accept: ItemTypes.CARD,

      fetch('/api/sheets')

        .then(res => res.json())    drop: (item: { patient: any }) => onDrop(item.patient, state),    collect: (monitor) => ({      isDragging: !!monitor.isDragging(),

        .then(data => {

          if (data && data.length > 1) {    collect: (monitor) => ({

            const headers = data[0].map((h: string) => h ? h.trim().toLowerCase() : '');

            const rows = data.slice(1).map((row: any, index: number) => {      isOver: !!monitor.isOver(),      isDragging: !!monitor.isDragging(),    }),
            ...
            if (!user) {
              ...
              setPatients([]);    <div ref={drop as unknown as React.Ref<HTMLDivElement>} className={`w-1/5 p-4 bg-gray-100 rounded-lg ${isOver ? 'bg-gray-200' : ''}`}>

              setIsLoading(false);

              return;      <h3 className="font-bold mb-4 text-center">{state}</h3>  return (
      ...
      {patients.map((patient, index) => (
            ...
            let filteredRows = rows;        <Card key={index} patient={patient} />  return (    <div
            ...
            if (user.email === 'eabarragang@ingenes.com') {
              ...
              filteredRows = rows.filter((p: any) => p[sucursalHeader]?.trim().toLowerCase() === user.sucursal?.toLowerCase());    </div>    <div      ref={drag as unknown as React.Ref<HTMLDivElement>}
          ...
          } else {};      ref={drag as unknown as React.Ref<HTMLDivElement>}      className={`p-4 mb-4 bg-white rounded-lg shadow-md ${isDragging ? 'opacity-50' : 'opacity-100'}`}
        ...
        .catch(() => {export default function KanbanPage() {      className={`p-4 mb-4 bg-white rounded-lg shadow-md ${isDragging ? 'opacity-50' : 'opacity-100'}`}    >

          toast({ title: 'Error', description: 'No se pudieron cargar los datos.', variant: 'destructive' });

          setPatients([]);  const [patients, setPatients] = useState<any[]>([]);

        })

        .finally(() => {  const [isLoading, setIsLoading] = useState(true);    >      <h4 className="font-bold">{`${patient.Nombre} ${patient.ApellidoP} ${patient.ApellidoM}`}</h4>
  ...
  }, [status, session, toast]);  useEffect(() => {      <p className="text-sm text-gray-600">{patient.email}</p>      <p className="text-sm text-gray-800">{patient.NHCDefinitivo}</p>



  const handleDrop = async (patient: any, newState: string) => {    const fetchData = () => {

    try {

      const response = await fetch('/api/sheets', {      setIsLoading(true);      <p className="text-sm text-gray-800">{patient.nhcdefinitivo}</p>    </div>

        method: 'POST',

        headers: { 'Content-Type': 'application/json' },      fetch('/api/sheets')

        body: JSON.stringify({

          rowIndex: patient.originalIndex,        .then(res => res.json())      <p className="text-xs text-gray-500 mt-2">{patient.sucursal}</p>  );

          header: 'estado',

          value: newState,        .then(data => {
      ...
      if (!response.ok) {            const headers = data[0].map((h: string) => h ? h.trim().toLowerCase() : '');
      ...
      toast({ title: 'Éxito', description: 'El estado del paciente ha sido actualizado.' });              const rowData: Record<string, any> = { originalIndex: index + 2 };

      setPatients(prev => prev.map(p => p.originalIndex === patient.originalIndex ? { ...p, estado: newState } : p));

    } catch (error) {              headers.forEach((header: string, i: number) => {};const Column = ({ state, patients, onDrop }: { state: string, patients: any[], onDrop: (p: any, state: string) => void }) => {
  ...
  if (status === 'loading' || isLoading) {
    ...
    return (              return rowData;

      <>

        <Header />            });const Column = ({ state, patients, onDrop }: { state: string, patients: any[], onDrop: (p: any, state: string) => void }) => {    accept: ItemTypes.CARD,
  ...
  return (              setPatients([]);    accept: ItemTypes.CARD,    collect: (monitor) => ({
    ...
    <>
      ...
      <DndProvider backend={HTML5Backend}>
        ...
        <div className="flex p-4 space-x-4 bg-gray-50" style={{height: 'calc(100vh - 4rem)'}}>              return;    drop: (item: { patient: any }) => onDrop(item.patient, state),      isOver: !!monitor.isOver(),
          ...
          {states.map(state => (
            ...
            <Column            }
              ...
              state={state}    collect: (monitor) => ({    }),

              patients={patients.filter(p => p.estado && p.estado.trim().toUpperCase() === state)}

              onDrop={handleDrop}            const sucursalHeader = 'sucursal';
